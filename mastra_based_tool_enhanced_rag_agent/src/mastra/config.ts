...
if (missingRequired.length > 0) {
  ...
  console.error(
    "Set them in a .env file in this project's root (see .env.example).",
  );
  process.exit(1);
}
...
export const CONFIG = {
  ...
  documentsCollection:
    process.env.CHROMA_COLLECTION_NAME || 'mastra_rag_documents',
  ...
  maxOutputTokens: Number(process.env.MAX_OUTPUT_TOKENS) || 4000,
  memoryDbUrl: process.env.MEMORY_DB_URL || 'file:./mastra-memory.db',
  memoryLastMessages: Math.max(
    1,
    Number(process.env.MEMORY_LAST_MESSAGES) || 20,
  ),
  memoryThreadId: process.env.MEMORY_THREAD_ID || 'cli-session',
  memoryResourceId: process.env.MEMORY_RESOURCE_ID || 'cli-user',
  reasoningEffort: (process.env.REASONING_EFFORT || 'low') as
    | 'xhigh'
    | 'high'
    | 'medium'
    | 'low'
