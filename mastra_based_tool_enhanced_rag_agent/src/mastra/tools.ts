...
import { formatRetrievedContext, hybridRetrieveDocuments } from "./rag/store.js";
...
export const getFlightScheduleTool = createTool({
  ...
  description: "Return a round-trip flight schedule between two cities and the total flight cost in USD.",
...
export const convertCurrencyTool = createTool({
  ...
  execute: async ({ amount, from_currency, to_currency }) => {
    ...
    if (!rate) {
      return { error: `Unsupported currency pair: ${from_currency} to ${to_currency}` };
    }
    return {
...
export const queryInternalKnowledgeBaseTool = createTool({
  ...
  execute: async ({ query, top_k }) => {
    try {
      ...
      return { error: `Internal knowledge base is currently unavailable: ${message}` };
    }
  },
});
