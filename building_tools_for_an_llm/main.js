...
async function completeConversation() {
  ...
  for (let turn = 0; turn < maxTurns; turn += 1) {
    ...
    if (toolCalls.length === 0) {
      const hasAllTools = requiredToolNames.every((name) => usedTools.has(name));
      if (hasAllTools && assistantMessage.content) {
        return assistantMessage.content;
      }

      const missingTools = requiredToolNames.filter((name) => !usedTools.has(name));
      messages.push({
        role: "user",
  ...
  throw new Error("The model did not complete the tool-calling conversation in time.");
}
