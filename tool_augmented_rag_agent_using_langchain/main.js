...
const CONFIG = {
  ...
  documentsCollection: process.env.CHROMA_COLLECTION_NAME || "tool_augmented_rag_documents",
  memoryCollection: process.env.CHROMA_MEMORY_COLLECTION_NAME || "tool_augmented_rag_conversations",
  ...
  memorySimilarityThreshold: clamp01(Number(process.env.MEMORY_SIMILARITY_THRESHOLD), 0.5),
...
function sentenceSegments(text) {
  ...
  if (typeof Intl !== "undefined" && Intl.Segmenter) {
    ...
    return Array.from(segmenter.segment(normalized), (segment) => segment.segment.trim()).filter(Boolean);
  }
...
async function ingestDataFolder(documentsStore) {
  ...
  for (const fileName of fileNames) {
    ...
    const ids = chunks.map((chunk, index) => createRecordId(fileName, index, chunk));
...
async function hybridRetrieveDocuments(documentsStore, query, k) {
  ...
  const candidates = await documentsStore.similaritySearchWithScore(query, overfetch);
...
async function recallConversationMemory(memoryStore, query, k) {
  const results = await memoryStore.similaritySearchWithScore(query, k).catch(() => []);
  return results
...
function convertCurrency({ amount, from_currency, to_currency }) {
  ...
  if (!rate) {
    throw new Error(`Unsupported currency pair: ${from_currency} to ${to_currency}`);
  }
...
function buildTools(documentsStore) {
  ...
  const getHotelBookingScheduleTool = tool(
    async ({ city, nights }) => JSON.stringify(getHotelBookingSchedule({ city, nights })),
    {
      name: "get_hotel_booking_schedule",
      description: "Return a hotel booking schedule for a stay and the total hotel cost in USD.",
      schema: z.object({
  ...
  const convertCurrencyTool = tool(
      ...
      JSON.stringify(convertCurrency({ amount, from_currency, to_currency })),
    {
      ...
      description: "Convert an amount from one currency to another using a deterministic exchange rate table.",
      schema: z.object({
  ...
  const queryInternalKnowledgeBaseTool = tool(
    async ({ query, top_k }) => {
      const items = await hybridRetrieveDocuments(documentsStore, query, top_k || CONFIG.topK);
    ...
    },
    {
      ...
      schema: z.object({
        query: z.string().describe("The search query to look up in the internal knowledge base"),
...
function describeMessage(message) {
  ...
  if (type === "ai") {
    ...
    if (toolCalls.length > 0) {
      const calls = toolCalls.map((call) => `  - ${call.name}(${JSON.stringify(call.args)})`).join("\n");
      const header = `[Assistant] requested ${toolCalls.length} tool call(s):`;
      return message.content ? `${header}\n${calls}\n${message.content}` : `${header}\n${calls}`;
    }
    return `[Assistant]\n${message.content}`;
  }
...
async function main() {
  ...
  const embeddings = new HuggingFaceInferenceEmbeddings({
    ...
    // Pin the provider explicitly: leaving this unset makes the HF client
    // resolve "auto" on every call and log its provider choice to stdout,
    // which would pollute the required conversation-history output.
  ...
  const llm = new ChatOpenAI({
    ...
    configuration: {
      ...
      // openai@4's Node shim defaults to node-fetch, whose gzip decompression
      // intermittently throws ERR_STREAM_PREMATURE_CLOSE on keep-alive
      // connections. Force Node's native fetch to avoid it.
  ...
  const recalled = await recallConversationMemory(memoryStore, prompt, CONFIG.memoryTopK);
  ...
  if (recalled.length > 0) {
    ...
    recalled.forEach(({ doc, similarity }, index) => {
      ...
      console.log(`Memory ${index + 1} (similarity=${similarity.toFixed(4)}, saved_at=${savedAt})`);
  ...
  const result = await agent.invoke(
    { messages },
    { configurable: { thread_id: "cli-session" }, recursionLimit: CONFIG.maxAgentSteps }
  );
...
main().catch((error) => {
  ...
  const looksLikeChromaConnectionError =
    /ECONNREFUSED|fetch failed|ChromaConnectionError|Failed to connect to chromadb/i.test(message);
