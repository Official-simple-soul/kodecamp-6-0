...
async function main() {
  ...
  const stage1Chain = createStageChain(loadPrompt("01_interpret_request.txt"), llm);
  const stage2Chain = createStageChain(loadPrompt("02_identify_possible_categories.txt"), llm);
  const stage3Chain = createStageChain(loadPrompt("03_select_best_category.txt"), llm);
  const stage4Chain = createStageChain(loadPrompt("04_extract_missing_requirements.txt"), llm);
  const stage5Chain = createStageChain(loadPrompt("05_generate_initial_assessment.txt"), llm);
